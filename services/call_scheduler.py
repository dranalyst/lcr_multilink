...
MAX_BATCH = int(os.getenv("SCHEDULER_MAX_BATCH", "2"))
...
def dispatch_due_asterisk_calls(db: Session) -> None:
    ...
    for sch in schedules:
        ...
        planned_duration = int(sch.planned_duration or 0)
        destination_gw = (sch.destination_gw or "").strip()
        ...
        logger.info(
            f"Schedule {sch.id}: provider={provider!r} "
            f"src={src!r} dst={dst!r} "
            f"planned_duration={planned_duration}s "
            f"destination_gw={destination_gw!r} "
        ...
        order = AsteriskCallOrder(
            ...
            exten="s",
            priority=1,
            planned_duration=sch.planned_duration,
            destination_gw=sch.destination_gw,
        )
